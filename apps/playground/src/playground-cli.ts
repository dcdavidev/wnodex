...
export class PlaygroundCLI {
  ...
  start(): void {
    ...
    this.#rl.on('line', async (line) => {
      // Pause readline to prevent prompt collision with async logs
      this.#rl.pause();

      await this.#handleInput(line);

      // Resume and show prompt for next command
      this.#rl.resume();
      this.#rl.prompt();
    });
  }
  ...
  async #handleStart(): Promise<void> {
    ...
    try {
      ...
      // Force a newline to ensure the next prompt appears on its own line
      console.log();
    } catch (error) {
